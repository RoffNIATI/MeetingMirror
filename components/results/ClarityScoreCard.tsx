...
export default function ClarityScoreCard({ clarity }: Props) {
  ...
  function scoreLabel(score: number): string {
    if (score >= 8) return t.scoreStrong;
    if (score >= 6) return t.scoreModerate;
    if (score >= 4) return t.scoreWeak;
    return t.scorePoor;
  }

  function clarityToText(c: MeetingClarity): string {
    return [
      `${t.cardClarity}: ${c.score}/10 — ${scoreLabel(c.score)}`,
      `\n${c.rationale}`,
  ...
  return (
    <CardShell title={t.cardClarity} copyText={clarityToText(clarity)} iconBg="bg-[var(--border)]" iconColor="text-[var(--muted)]"
      ...
      <div className="flex items-end gap-3 mb-3">
        <span className="font-display font-bold text-5xl leading-none" style={{ color }}>{clarity.score}</span>
        <div className="pb-1">
      ...
      {clarity.strengths.length > 0 && (
        <div className="mb-3">
          <p className="text-xs font-semibold text-[var(--text)] mb-1.5 uppercase tracking-wide">{t.strengths}</p>
          <ul className="space-y-1">
            {clarity.strengths.map((s, i) => (
              <li key={i} className="flex items-start gap-2 text-xs text-[var(--text)]">
                <svg width="12" height="12" viewBox="0 0 12 12" fill="none" className="mt-0.5 shrink-0"><path d="M2 6l2.5 2.5 5.5-5" stroke="#16A34A" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round"/></svg>
                {s}
              </li>
            ))}
          </ul>
      ...
      {clarity.gaps.length > 0 && (
        <div className="mb-3">
          <p className="text-xs font-semibold text-[var(--text)] mb-1.5 uppercase tracking-wide">{t.gaps}</p>
          <ul className="space-y-1">
            {clarity.gaps.map((g, i) => (
              <li key={i} className="flex items-start gap-2 text-xs text-[var(--text)]">
                <svg width="12" height="12" viewBox="0 0 12 12" fill="none" className="mt-0.5 shrink-0"><path d="M3 9l6-6M9 9L3 3" stroke="#D97706" strokeWidth="1.4" strokeLinecap="round"/></svg>
                {g}
              </li>
            ))}
          </ul>
      ...
      {clarity.recommendations.length > 0 && (
        <div>
          <p className="text-xs font-semibold text-[var(--text)] mb-1.5 uppercase tracking-wide">{t.recommendations}</p>
          <ul className="space-y-1">

...
export default function UnresolvedCard({ unresolvedItems }: Props) {
  ...
  const typeLabel: Record<string, string> = {
    open_question: t.openQuestion, ambiguity: t.ambiguity,
    unresolved_conflict: t.conflict, missing_information: t.missingInfo,
  };
  const impactLabel: Record<Severity, string> = {
    high: t.highImpact, medium: t.mediumImpact, low: t.lowImpact,
  };

  function itemsToText(items: UnresolvedItem[]): string {
    return items.map((u, i) => `${i + 1}. [${typeLabel[u.type] || u.type}] ${u.description} (Impact: ${u.impact})`).join('\n');
  }

  return (
    <CardShell title={t.cardUnresolved} copyText={itemsToText(unresolvedItems)} iconBg="bg-amber/10" iconColor="text-amber"
          ...
          {unresolvedItems.map((u, i) => (
            <li key={i} className="flex items-start gap-2.5 py-2.5 border-b border-[var(--border)] last:border-0 last:pb-0">
              <div className="flex flex-wrap gap-1.5 shrink-0 mt-0.5">
                ...
                <Badge label={impactLabel[u.impact]} variant={impactVariant[u.impact]} size="xs" />
              </div>

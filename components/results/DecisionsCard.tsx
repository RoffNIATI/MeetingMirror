...
export default function DecisionsCard({ decisions }: Props) {
  return (
    <CardShell
      title="Decisions"
      copyText={decisionsToText(decisions)}
      iconBg="bg-green/10"
    ...
    >
      {decisions.length === 0 ? (
        <p className="text-sm text-[var(--muted)] italic">No explicit decisions were identified.</p>
      ) : (
        <ul className="space-y-3">
          {decisions.map((d, i) => (
            <li key={i} className="flex items-start gap-2.5">
              <Badge
                label={d.status === 'confirmed' ? 'Confirmed' : 'Likely'}
                variant={d.status === 'confirmed' ? 'green' : 'amber'}
                size="xs"
              />
              <div className="flex-1 min-w-0">
                <p className="text-sm text-[var(--text)] leading-snug">{d.text}</p>
                {d.context && (
                  <p className="text-xs text-[var(--muted)] mt-0.5 italic">{d.context}</p>
                )}
              </div>
            </li>
          ))}
